...
<%
    ...
    boolean pIsDirectTrade = false; // ✨ [추가] 직거래 여부 변수
    ...
    try {
        ...
        if (rs.next()) {
            ...
            pIsDirectTrade = rs.getBoolean("is_direct_trade"); // ✨ [추가] DB에서 값 가져오기
...
<head>
    ...
    <style>
        ...
        .product-form table { width: 100%; border-collapse: collapse; border-top: 2px solid #333; }
        .product-form th, .product-form td { padding: 15px; border-bottom: 1px solid #eee; }
        .product-form th { width: 150px; background-color: #fcfcfc; text-align: left; vertical-align: top; font-weight: 500; }
        .product-form input[type="text"], .product-form input[type="number"], .product-form select, .product-form textarea { width: 100%; padding: 10px; font-size: 15px; border: 1px solid #ccc; border-radius: 5px; box-sizing: border-box; }
        .product-form textarea { height: 200px; resize: vertical; }
        .product-form input[type="file"] { font-size: 14px; margin-top: 5px; }
        .current-images { display: flex; gap: 10px; margin-bottom: 15px; }
        .current-images .img-preview { width: 100px; height: 100px; object-fit: contain; border: 1px solid #ddd; background-color: #f9f9f9; border-radius: 4px; }
        .current-images-label { font-size: 13px; color: #777; margin-bottom: 5px; }
        .image-upload-area div { margin-bottom: 10px; }
        .image-upload-area label { font-weight: 500; font-size: 14px; color: #555; }
        .shipping-option label { margin-right: 15px; font-size: 15px; }
        .shipping-option input[type="radio"] { margin-right: 5px; }
                ...
                <table>
                    <tbody>
                        ...
                        <tr>
                            <th>상품명</th>
                            <td>
                                <input type="text" name="product_name" value="<%= pName %>" required>
                            </td>
                        </tr>
                        <tr>
                            <th>가격</th>
                        ...
                        <tr>
                            ...
                            <td class="shipping-option">
                                <input type="radio" name="shipping_included" value="false" id="ship_no" <%= (!pShippingIncluded ? "checked" : "") %>>
                                <label for="ship_no">택배비 미포함 (X)</label>
                                
                                <input type="radio" name="shipping_included" value="true" id="ship_yes" <%= (pShippingIncluded ? "checked" : "") %>>
                        ...
                        <tr>
                            <th>직거래 여부</th>
                            <td class="shipping-option">
                                <input type="radio" name="is_direct_trade" value="false" id="direct_no" <%= (!pIsDirectTrade ? "checked" : "") %>>
                                <label for="direct_no">직거래 불가 (X)</label>
                                
                                <input type="radio" name="is_direct_trade" value="true" id="direct_yes" <%= (pIsDirectTrade ? "checked" : "") %>>
                                <label for="direct_yes">직거래 가능 (O)</label>
                            </td>
                        </tr>
