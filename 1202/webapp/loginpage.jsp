...
body { margin: 0; font-family: 'Noto Sans KR', sans-serif; background-color: #f9f9f9; color: #333; display: flex; flex-direction: column; min-height: 100vh; }
...
.show { display: block; }
.login-section { flex-grow: 1; display: flex; justify-content: center; align-items: center; padding: 80px 0; }
.login-section-content { max-width: 400px; padding: 30px 40px; background-color: #ffffff; border-radius: 10px; box-shadow: 0 5px 15px rgba(0, 0, 0, 0.1); width: 100%; box-sizing: border-box; }
.login-section h2 { display: none; }
form.login-form-box { display: flex; gap: 10px; }
.input-fields { flex-grow: 1; display: flex; flex-direction: column; gap: 10px; }
.input-fields input[type="text"], .input-fields input[type="password"] { padding: 12px 10px; border: 1px solid #ccc; border-radius: 5px; font-size: 14px; }
input.login-submit-btn { width: 100px; height: 95px; padding: 10px; background-color: #81c147; color: white; border: none; border-radius: 5px; font-size: 16px; font-weight: bold; cursor: pointer; transition: background-color 0.3s ease; box-sizing: border-box; }
input.login-submit-btn:hover { background-color: #70a93d; }
.extra-options { margin-top: 20px; text-align: center; font-size: 14px; padding-bottom: 20px; border-bottom: 1px solid #eee; display: flex; justify-content: space-between; }
.extra-options a { color: #333; text-decoration: none; font-weight: 500; }
.extra-options a:hover { text-decoration: underline; }
.social-login-buttons { display: flex; flex-direction: column; gap: 10px; margin-top: 20px; }
.social-login-buttons button, .social-login-buttons a { padding: 12px; border: 1px solid #ccc; border-radius: 5px; font-size: 14px; cursor: pointer; font-weight: bold; transition: background-color 0.3s ease; text-align: center; text-decoration: none; }
.btn-naver { background-color: #03C75A; border-color: #03C75A; color: #FFFFFF; }
.btn-naver:hover { background-color: #02b350; }
.btn-kakao { background-color: #FEE500; border-color: #FEE500; color: #000000; }
.btn-kakao:hover { background-color: #e5cd00; }
.logged-in-info { text-align: center; font-size: 16px; margin-bottom: 20px; }
.logged-in-actions { display: flex; gap: 10px; justify-content: center; }
.logged-in-actions input[type="submit"], .logged-in-actions input[type="button"] { padding: 10px 15px; border: none; border-radius: 5px; cursor: pointer; font-size: 14px; font-weight: bold; }
.logged-in-actions input[type="submit"] { background-color: #f44336; color: white; }
.logged-in-actions input[type="button"] { background-color: #2c7be5; color: white; }
...
function validateForm() {
    ...
    if (id == "" && pw == "") { alert("아이디와 비밀번호를 모두 입력하시오."); return false; } 
    else if (id == "") { alert("아이디를 입력하시오."); return false; } 
    else if (pw == "") { alert("비밀번호를 입력하시오."); return false; }
    return true;
}
...
<body>
    ...
    <div class="login-section">
        ...
        <% if ((String)session.getAttribute("userId") != null) { %>
            ...
            <form action="member" method="post" class="logged-in-actions">
        ...
        <% } else { %>
            <h2>로그인</h2>
            <form action="member" method="post" class="login-form-box" onsubmit="return validateForm()">
            ...
            <div class="extra-options">
                ...
                <a href="#" onclick="window.open('find_account_form.jsp', 'findAccount', 'width=500,height=550'); return false;">아이디/비밀번호 찾기</a>
            </div>
