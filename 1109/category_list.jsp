...
<%
    ...
    String categoryName = "상품 목록"; 
    boolean hasProducts = false;

    if (categoryId == null || categoryId.trim().isEmpty()) {
        out.println("<script>alert('카테고리 ID가 없습니다.'); history.back();</script>");
        return;
    }
    ...
    try {
        ...
        String sqlCategory = "SELECT category_name FROM category WHERE category_id = ?";
        ...
        if (rs.next()) {
            ...
            out.println("<script>alert('존재하지 않는 카테고리입니다.'); history.back();</script>");
            return;
        }
...
<body>
    ...
    <section class="recommend-section">
        ...
        <div class="product-grid">
            <%
                try {
                    String sqlProducts = "SELECT product_id, product_name, price, main_image_url FROM Product " +
                                         "WHERE category_id = ? AND is_sold_out = FALSE ORDER BY created_at DESC";
                    
                    pstmt = conn.prepareStatement(sqlProducts);
                    pstmt.setString(1, categoryId);
                    rs = pstmt.executeQuery();
            ...
            <%
                    ...
                    if (!hasProducts) {
                        out.println("<p>이 카테고리에 등록된 상품이 아직 없습니다.</p>");
                    }
