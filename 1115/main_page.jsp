...
<head>
    ...
    <style>
        ...
        /* 기존의 .search-bar input 스타일 (일반 input용) */
        .search-bar input {
            /* 이 스타일은 input[name="query"]에 의해 덮어쓰여집니다. */
            width: 100%;
            padding: 12px 16px;
        ...
        /* 💡 수정된 검색 폼 (Form) 스타일: 중앙 정렬 및 최대 너비 설정 */
        .search-bar form { 
            display: flex;
            width: 100%;
            max-width: 500px; /* 여기서 검색창의 전체 너비를 제한합니다. */
            position: relative; /* X 버튼 위치 지정을 위해 추가 */
        }

        /* 💡 수정된 검색어 입력 필드 스타일 재정의 */
        .search-bar input[name="query"] {
            flex-grow: 1;
            width: auto; /* Flexbox가 너비를 관리하도록 설정하여 삐져나오는 문제 해결 */
            ...
            font-size: 16px;
            height: 44px;
            box-sizing: border-box; /* 패딩과 보더가 너비에 포함되도록 하여 정확한 레이아웃 유지 */
        }
        
        /* 💡 X 버튼 스타일 */
        .clear-search {
            position: absolute;
            right: 80px; /* 검색 버튼 너비(44px) + 약간의 간격 확보 */
            top: 50%;
        ...
        .search-bar button.search-button {
            ...
            height: 44px; /* input의 높이 (40px) + border (2px) * 2 = 44px에 맞춤 */
            line-height: 1;
            display: flex;
            align-items: center;
            justify-content: center;
...
<body>
    ...
    <script>
        ...
        clearSearchBtn.addEventListener('click', function() {
            searchInput.value = '';
            this.style.display = 'none'; // 삭제 후 X 버튼 숨김
            searchInput.focus(); // 입력 필드에 포커스 유지
        });
        // 3. (참고) Enter 키는 form의 기본 동작으로 검색이 실행됩니다.
    </script>
