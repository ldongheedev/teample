...
<%
    ...
    if (categoryId == null) {
        categoryId = "all"; // 기본값 설정 (전체 카테고리)
    }

    // 카테고리 ID를 한글 이름으로 변환하는 맵
    HashMap<String, String> categoryNames = new HashMap<>();
    categoryNames.put("all", "전체 카테고리");
    categoryNames.put("clothing", "의류");
    categoryNames.put("food", "식품");
    categoryNames.put("accessory", "액세서리");
    categoryNames.put("digital", "디지털/가전제품");
    categoryNames.put("sports", "스포츠 용품");
    categoryNames.put("pet", "애완동물 용품");
    categoryNames.put("talent", "재능");

    String categoryName = categoryNames.getOrDefault(categoryId, "알 수 없는 카테고리");
%>
...
<head>
    <meta charset="UTF-8">
    <title><%= categoryName %> 목록 - 중고모아</title>
    ...
    <style>
        ...
        /* 💡 요청하신 최종 CSS 스타일 적용 (height: 44px 및 우측 패딩) */
        .search-bar input[name="query"] {
            flex-grow: 1;
            width: auto;
            padding: 10px 40px 10px 15px; /* X 버튼 공간 확보를 위해 오른쪽 패딩 증가 */
            border: 2px solid #81c147; 
            border-right: none;
            border-radius: 8px 0 0 8px;
            outline: none;
            font-size: 16px;
            height: 44px; /* 높이 44px로 통일 */
        ...
        .category-section h2 {
            ...
            margin-bottom: 20px;
            color: #333;
            border-bottom: 2px solid #81c147;
            padding-bottom: 10px;
        }
    ...
    <div class="search-area-container">
        ...
        <div class="search-bar">
            <form action="search_result.jsp" method="get" class="search-form" id="searchForm">
                <input type="text" name="query" id="searchInput" placeholder="<%= categoryName %>에서 검색" required />
    ...
    <section class="category-section">
        ...
        <div class="product-grid">
            <%
                ...
                // 검색할 SQL 쿼리 준비
                String sql = "SELECT product_id, product_name, price, main_image_url FROM Product WHERE is_sold_out = FALSE ";
            ...
            <%
                    ...
                    if (!hasProducts) {
                        // ❗ JSP 컴파일 오류 수정: 문자열 연결 연산자(+) 사용
                        out.println("<p>현재 " + categoryName + "에 등록된 상품이 없습니다.</p>");
                    }
