...
<%
    ...
    try {
        ...
        if (rs.next()) {
            ...
            addrBase = (rs.getString("addr_base") != null) ? rs.getString("addr_base") : "";
...
<head>
    ...
    <style>
        ...
        /* zipcode-btn 스타일을 인증 버튼에도 공용으로 사용 */
        .zipcode-btn {
            padding: 10px 15px;
    ...
    <script>
        ...
        // --- [수정됨] ---
        // 1. 최종 폼 제출 시, 전화번호 변경 여부 및 인증 여부 확인
        function validateUpdateForm() {
            ...
            // --- [추가된 로직] ---
            // 전화번호가 변경되었는데, 인증(phone_verified)이 'true'가 아니면 제출 막기
        ...
        function formatPhone(input) {
            ...
            if (currentPhone !== originalPhone) {
                verifiedStatus.value = "false"; // 인증 상태 초기화
                // 인증 관련 UI 초기화 (버튼/입력창 활성화)
                ...
                // 원본 전화번호와 같아졌다면 인증 버튼/영역 숨김
                btnSend.style.display = "none";
                authArea.style.display = "none";
                verifiedStatus.value = "false"; // 원본과 같으면 인증이 필요 없음 (제출 시 통과됨)
            }
            // --- [여기까지 추가] ---
        }
        
        // --- [신규 추가] ---
        // 3. 인증번호 발송 (클라이언트에서 난수 생성 및 alert)
        function sendAuthCode() {
            var code = Math.floor(1000 + Math.random() * 9000); // 1000 ~ 9999 사이 난수
            document.getElementById("server_auth_code").value = code; // 생성된 코드를 숨겨진 필드에 저장
            
            // 실제 SMS 대신 alert으로 인증번호 알림
        ...
        function confirmAuthCode() {
            ...
            if (serverCode !== "" && serverCode === userCode) {
                alert("인증에 성공했습니다.");
                document.getElementById("phone_verified").value = "true"; // 인증 상태 'true'로 변경
                
                // 성공 시 더 이상 수정/재전송 못하게 비활성화
                ...
                document.getElementById("server_auth_code").value = ""; // 서버 코드 초기화 (보안상)
            }
        }
    </script>
</head>
        ...
        <nav class="mypage-sidebar">
            ...
            <h3>상품관리</h3>
             ...
             <ul>
                ...
                <li><a href="product_edit.jsp">상품 정보 수정</a></li>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="button" value="인증번호" id="btn_send_code" class="zipcode-btn"
                                   style="display:none; margin-left:10px; background-color:#2c7be5;" 
                                   onclick="sendAuthCode()">
                                
                                <div id="auth_code_area" style="display:none; margin-top:10px;">
                                    <input type="text" id="user_auth_code" placeholder="인증번호 4자리 입력" style="width: 160px; margin-right: 10px;">
